...
SSR-streaming benchmarks will be appended after the stable Vite build is
validated in RSF.
...
1. Update RSF to the validated upstream commit and compare production
   Lighthouse, Core Web Vitals, transferred bytes, and request waterfalls.
2. Benchmark SSR streaming separately on anonymous public routes, retaining
   non-streaming behavior for authenticated, protected, redirect, and error
   routes until header and cache correctness is proven.
3. Evaluate Rspack as a separate compatibility and build-speed experiment; it
   is not the shared default during the Nuxt 4.5 adoption.
