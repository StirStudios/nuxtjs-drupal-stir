...
- N1 has started with a checksum-protected Stir Tools v1 snapshot, schema validation, aligned registration/Webform types, administrator-approval handling, and safe Drupal Webform confirmation redirects. The synchronized compatibility manifest validates Drupal/PHP requirements, optional provider-capability ownership, and independent Nuxt/Stir Tools releases. App-context output is parsed through a shared production boundary that normalizes Drupal's empty-array blocks compatibility shape; global SEO now validates resolved head attributes through the same producer-owned approach before UI code.
