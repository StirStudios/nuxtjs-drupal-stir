...
- N1 has started with a checksum-protected Stir Tools v1 snapshot, schema validation, aligned registration/Webform types, administrator-approval handling, and safe Drupal Webform confirmation redirects. The synchronized compatibility manifest validates Drupal/PHP requirements, optional provider-capability ownership, and independent Nuxt/Stir Tools releases. Registration policy now validates Drupal's allowed modes and fails closed; app-context normalizes Drupal's empty-array blocks compatibility shape; global SEO validates resolved head attributes before UI code.
