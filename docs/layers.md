...
- Every capability layer reaches `layers/foundation`, which registers `@nuxtjs/robots` and application mode, so a standalone composition is never indexable without the SEO layer. Editorial and Integrations extend `layers/platform` because they use its `#stir/*` utilities; Listing and SEO extend `layers/foundation` directly; Auth and Webforms reach it through `layers/turnstile` and `layers/platform`.
