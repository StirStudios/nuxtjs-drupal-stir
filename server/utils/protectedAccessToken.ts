...
const sign = async (data: string, secret: string): Promise<string> =>
  encodeBytesBase64Url(await signatureBytes(data, secret))

export const stirConstantTimeEquals = (first: string, second: string): boolean => {
...
export const createStirProtectedAccessToken = async (
  secret: string,
...
export const verifyStirProtectedAccessToken = async (
  ...
  if (!stirConstantTimeEquals(receivedSignature, expectedSignature)) return false
