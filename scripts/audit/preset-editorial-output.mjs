...
async function outputEvidence(name) {
  ...
  return { adminCss, markerFiles }
}

const minimal = await outputEvidence('minimal')
const full = await outputEvidence('full')

if (minimal.adminCss.length > 0 || minimal.markerFiles.length > 0) {
  throw new Error('Minimal production output contains editorial admin UI assets.')
}

if (full.adminCss.length === 0 || full.markerFiles.length === 0) {
  throw new Error('Full production output is missing editorial admin UI assets.')
}
